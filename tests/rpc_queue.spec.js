...
test('rpc queue throttles and respects single-flight ordering', async ({ page }) => {
  ...
  // Account for read-loop phase jitter: nextLine delay might already be substantially elapsed
  // when the first queued RPC is written, especially on faster CI runners.
  const expectedDelay = Math.max(0, Math.floor((measurements.responseDelay ?? 0) * 0.1));
