...
For the operator-facing explanation of `Immediate local response` versus the browser-only pickup guard, read [Operator Tutorial](../docs/guides/OperatorTutorial.md).
...
- **Power Safety Summary** – Before a successful handshake the App says **Power status unavailable** and never treats the bundled fallback manifest as attached-device truth. After connection, Stage and Lab may show the device-reported `power_profile`, `led_brightness_cap`, and `rail_topology_verified` values as technical metadata. Only `POWER_CHOKED_V1` triggers the visible power-limited warning; `SPLIT_RAIL_REWORK` and verified-rail reports remain neutral metadata.
