...
- **Firmware Service Tab** – The **Service** utility tab is the browser-side handoff for firmware lifecycle work. It does not pretend to flash the board itself; instead it surfaces preflight state (connection, firmware identity, dirty staged edits, backup readiness, session-log count), gives you one-click exports for config JSON and the audit log, exposes the existing configurator reboot request, and then points you at the two real flash lanes: release hex via `teensy_loader_cli` or source upload via `pio run -d firmware -t upload -e teensy40_main`.
- **Live Runtime Controls** – The utility rail now hosts browser-driven runtime controls that do not participate in staged Apply/Rollback. `USB MIDI Output`, `Note Dynamics`, and `Jitter` talk straight to `GET_/SET_` protocol lanes, mirror telemetry readback, and use soft takeover on the firmware side so the hardware pots can reclaim control without a jump.
