...
- **Device Clock & Diagnostics** – The MIDI utility tab now separates browser-generated Web MIDI clock from the firmware’s own clock engine. `Device Clock` edits the hardware clock source, internal BPM, and clock-out flag live, while the `Device Monitor` now consumes live telemetry for clock state and loop/MIDI overrun counters instead of only showing static manifest metadata.
