...
- **Simulator Controls** – Configure's empty-state **Start simulator** action swaps WebSerial for the canned transport and connects in one step. Lab retains the explicit transport toggle. Because the simulator obeys the same throttled paint loop documented in [Runtime Contract](#runtime-contract), you can chase layout timing bugs or automation macros without a Teensy on the desk. Its deterministic EF rehearsal signals include active phrases, quiet floors, and periodic threshold crossings; simulated `efStatus[]` is derived from those same levels so scope state and motion agree.
...
- The Lab Scope keeps a separate rolling history for every manifest-advertised envelope follower. **EF traces: Active** uses firmware `efStatus[]` plus a 1.5-second visual hold so transient movement fades instead of disappearing; **All** reveals every EF trace. LFOs remain visible in both modes as secondary dashed traces. The 2s/5s/10s Window control changes the host-observed elapsed-time X axis without clearing history. The persistent view line names Active/All and any temporary EF focus, while **Leave solo** returns to the underlying view. Inactive histories continue recording, and the no-active-EF state says so explicitly.
- Scope X time is host-observed telemetry arrival time: it measures the spacing between App callbacks after browser, WebSerial, Bridge, and frame-assembly behavior. It is intended for musical visualization, not source-device or round-trip latency measurement. Use the [bench latency method](../docs/bench/latency/method.md) for latency-grade evidence.
